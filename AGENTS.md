...
- 品質: `pnpm lint` / `pnpm lint:fix` / `pnpm markuplint` / `pnpm fmt` / `pnpm fmt:check`

## コーディング規約・命名

- 言語: TypeScript（厳格設定）/ React / Next.js 16 / Tailwind CSS
- フォーマット（oxfmt）: 2 スペース, 100 文字幅, セミコロン有, シングルクォート, 末尾カンマ
...
- Issue がある場合は、ブランチ名と PR タイトルに `#XX` のように Issue 番号を含める
